...
import (
	"context"
	"fmt"
	"github.com/JingdaMai/bookstore_utils-go/logger"
...
func (c *esClient) Get(index string, docType string, id string) (*elastic.GetResult, error) {
	...
	result, err := c.client.Get().
		...
		Do(ctx)
	if err != nil {
		logger.Error(fmt.Sprintf("error when tyring to get id %s", id), err)
		return nil, err
	}
	return result, nil
}
...
func (c *esClient) Search(index string, query elastic.Query) (*elastic.SearchResult, error) {
	...
	result, err := c.client.
		...
		Do(ctx)
	if err != nil {
		logger.Error(fmt.Sprintf("error when trying to search document in %s", index), err)
		return nil, err
	}
	return result, nil
